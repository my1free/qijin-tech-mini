...
export default {
  ...
  computed: {
    ...
    ctrlStyle: function() {
      ...
      if (this.maskType !== "outline") {
        cssStr += `box-shadow: 0 0 0 50000rpx rgba(0,0,0, ${
          this.view ? 0.8 : 0.4
        })`;
      } else {
  ...
  methods: {
    toPx(str) {
      if (str.indexOf("%") !== -1) {
        return Math.floor(
          (Number(str.replace("%", "")) / 100) * this.containerWidth
        );
      }
    ...
    resetCutReact() {
      ...
      if (this.cutHeight) {
        this.ctrlHeight = Math.min(
          this.toPx(this.cutHeight),
      ...
      const cornerStartX = this.center
        ? Math.floor((this.containerWidth - this.ctrlWidth) / 2)
        : 0;
      const cornerStartY = this.center
        ? Math.floor((this.containerHeight - this.ctrlHeight) / 2)
        : 0;
    ...
    async initImage() {
      ...
      this.image.height = err
        ? this.ctrlHeight
        : (res.height / res.width) * this.image.width;
      this.img = res.path;
      ...
      this.transform.angle =
        this.freeBoundDetect || !this.disableRotate ? this.angle : 0;
      ...
      if (e.touches.length > 1) {
        ...
        if (this.preVector.x !== null) {
          if (this.startDistance) {
            ...
            this.cutMode && !this.disableCtrl
              ? this.setCut()
              : !this.disableScale && this.pinch();
          }
          // rotate
          this.params.angle = calcAngle(v, this.preVector);
          this.cutMode && !this.disableCtrl
            ? this.setCut()
            : !this.disableRotate && this.rotate();
        }
        ...
        this.cutMode && !this.disableCtrl
          ? this.setCut()
          : !this.disableTranslate && this.translate();
    ...
    setZoom(scale) {
      scale = Math.min(
        Math.max(Number(scale) || 1, this.minZoom),
        this.maxZoom
      );
    ...
    setTranslate(offset) {
      if (Array.isArray(offset)) {
        ...
        this.transform.translate.x = isNaN(x)
          ? this.transform.translate.x
          : this.corner.left + x;
        this.transform.translate.y = isNaN(y)
    ...
    setBoundary() {
      ...
      if (!this.boundDetect || (!this.disableRotate && !this.freeBoundDetect))
        return true;
      ...
      const minX =
        ...
        this.image.width +
        this.ctrlWidth -
        (this.image.width * (zoom - 1)) / 2;
      const maxX = corner.left + (this.image.width * (zoom - 1)) / 2;
      const minY =
        corner.top -
        this.image.height +
        this.ctrlHeight -
        (this.image.height * (zoom - 1)) / 2;
      const maxY = corner.top + (this.image.height * (zoom - 1)) / 2;
      translate.x = Math.floor(
        translate.x < minX ? minX : translate.x > maxX ? maxX : translate.x
      );
      translate.y = Math.floor(
        translate.y < minY ? minY : translate.y > maxY ? maxY : translate.y
      );
    },
    preview() {
      clearTimeout(this.previewTimer);
    ...
    draw() {
      ...
      context.fillRect(
        0,
        0,
        this.ctrlWidth * canvasZoom,
        this.ctrlHeight * canvasZoom
      ); // clear canvas
      context.translate(
      ...
      context.translate(
        -img.width * zoom * 0.5 * canvasZoom,
        -img.height * zoom * 0.5 * canvasZoom
      );
      context.drawImage(
        this.img,
        0,
        0,
        img.width * zoom * canvasZoom,
        img.height * zoom * canvasZoom
      );
      context.restore();
